...
#        picked_clients, priority_clients = self.client_manager.resampleClients(select_num_participants, cur_time=self.global_virtual_clock, time_window=time_window)
    ...
    def client_completion_handler(self, results): #, importance=1.0):
        ...
        if self.round_update and not isStale and not args.send_delta:
            if not self.args.feddc:
                ...
                if clientId not in self.h_dict:
                    self.h_dict[clientId] = [0 for idx, param in enumerate(self.model.parameters())]
                else:
                    self.h_dict[clientId] = [self.h_dict[clientId][idx] + results['model_delta'][idx] for idx, param in enumerate(self.model.parameters())]

                if len(self.model_in_update) == 0:
                    self.model_in_update = [True]
                    for idx, param in enumerate(self.model.parameters()):
                        param.data = torch.from_numpy(results['update_weight'][idx] + self.h_dict[clientId][idx]).to(device=self.device) * self.client_importance[clientId]
                        self.count_param_updates[idx] += 1
                else:
                    for idx, param in enumerate(self.model.parameters()):
                        param.data += torch.from_numpy(results['update_weight'][idx] + self.h_dict[clientId][idx]).to(device=self.device) * self.client_importance[clientId]
    ...
    def round_completion_handler(self, importance=1.0):     
        ...
        if str(args.wandb_tags).startswith('safa'): #args.exp_type == 4:
            ...
            # self.total_worker = args.total_worker
        else:
        ...
        if self.args.stale_update != 0 or self.args.prohibit_reselect != 0:
            ...
            while len(re_sel_client) > 0:
                ...
                # index = self.rng.randint(0, len(self.online_feasible_clients) - 1)
                # cid = self.online_feasible_clients[index]                
                trials = 10
    ...
    def log_round_metrics(self):
        ...
        if args.use_wandb and self.epoch > 1:
            ...
            wandb.log({"Fairness/jain_selection": (1.0 / len(clients_selected) * (np.sum(clients_selected) ** 2) / np.sum(clients_selected ** 2)),
                      ...
                      "Fairness/jain_fail": (1.0 / len(clients_fail) * (np.sum(clients_fail) ** 2) / np.sum(clients_fail ** 2)),   #为了stage3屏蔽
                       "Fairness/qoe_fail": (1.0 - (2.0 * clients_fail.std() / (clients_fail.max() - clients_fail.min()))),        #为了stage3屏蔽
                       }, step=self.epoch)
            ...
            if len(self.event_queue) != 0:
                ...
                if event_msg == 'update_model':
                    ...
                    for executorId in self.executors:
                        ...
                        # logging.info(f'Update event_msg == start_round in aggregator.py of stage3 at {self.global_virtual_clock}, client_run_queue is {self.resource_manager.client_run_queue}')
                        if next_clientId is not None:

                            if self.args.feddc == 1:  
                                config = self.get_client_conf(next_clientId)
                                
                                if next_clientId not in self.h_dict:
                                    self.server_event_queue[executorId].put({'event': 'train', 'clientId':next_clientId, 'conf': config, 'feddc_h': None}) 
                                else:
                                    self.server_event_queue[executorId].put({'event': 'train', 'clientId':next_clientId, 'conf': config, 'feddc_h': self.h_dict[next_clientId]})
                                    # self.server_event_queue[executorId].put({'event': 'train', 'clientId':next_clientId, 'conf': config, 'feddc_h': -1})
                            else:
                                config = self.get_client_conf(next_clientId)
                ...
                if event_msg == 'train_nowait':
                    ...
                    next_clientId = self.resource_manager.get_next_task()
                    # logging.info(f'Update event_msg == train_nowait in aggregator.py of stage3 at {self.global_virtual_clock}, client_run_queue is {self.resource_manager.client_run_queue}')
                    if next_clientId is not None:
                        ...
                        if self.args.feddc == 1:  
                            if next_clientId not in self.h_dict:
                                runtime_profile = {'event': 'train', 'clientId':next_clientId, 'conf': config, 'feddc_h': None}
                                # self.server_event_queue[executorId].put(runtime_profile)
                            else:
                                runtime_profile = {'event': 'train', 'clientId':next_clientId, 'conf': config, 'feddc_h': self.h_dict[next_clientId]}
                                # self.server_event_queue[executorId].put(runtime_profile)
                        else:
                            runtime_profile = {'event': 'train', 'clientId':next_clientId, 'conf': config, 'feddc_h': None}
                            # self.server_event_queue[executorId].put(runtime_profile)
