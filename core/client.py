...
class Client(object):
    ...
    def train(self, client_data, model, conf, round_num=0, feddc_h=None): 
        ...
        Feddc_global_model = [param.data.clone() for param in model.parameters()]
            ...
            try:
                ...
                for data_pair in client_data:                       
                    ...
                    if conf.task == 'nlp':                          
                        ...
                        if conf.feddc == 1:
                            if feddc_h == None:
                                delta_R = [(param.data - Feddc_global_model[idx].data).cpu().numpy() for idx, param in enumerate(model.parameters())]
                            else:
                                delta_R = [(feddc_h[idx] + param.data - Feddc_global_model[idx].data).cpu().numpy() for idx, param in enumerate(model.parameters())]


                            # delta_R = [(self.h[idx] + param.data - Feddc_global_model[idx].data).cpu().numpy() for idx, param in enumerate(model.parameters())]
                            l2_norm_squared = sum(np.sum(arr ** 2) for arr in delta_R)                           

                            #loss += np.linalg.norm(delta_R) * conf.feddc_alpha
                            loss += l2_norm_squared * conf.feddc_alpha
                        ...
                        if conf.feddc == 1:
                            ...
                            if feddc_h == None:
                                ...
                                for idx, param in enumerate(model.parameters()):
                                    feddc_tensor = feddc_h[idx]
                                    if not isinstance(feddc_tensor, torch.Tensor):
                                        feddc_tensor = torch.tensor(feddc_tensor, device=param.device)
                                    diff = feddc_tensor + param.data - Feddc_global_model[idx].data
                                    delta_R.append(diff.cpu().numpy())

                            # delta_R = [(self.h[idx] + param.data - Feddc_global_model[idx].data).cpu().numpy() for idx, param in enumerate(model.parameters())]
                            l2_norm_squared = sum(np.sum(arr ** 2) for arr in delta_R)

                            loss = loss + torch.tensor(0.5*l2_norm_squared * conf.feddc_alpha, device=loss.device)
        ...
        if conf.feddc == 1:
            model_delta = [(param.data - Feddc_global_model[idx].data).cpu().numpy() for idx, param in enumerate(model.parameters())]
