...
class clientManager(object):
    ...
    # - return the availability of a client in a certain time window (time slots = deadline)  
    ...
    # - return the priority of a client based on its availability in a certain time window (time slots = deadline)
    ...
    def resampleClients(self, numOfClients, cur_time=0, time_window=0):
        ...
        logging.info(f'Feasible clients (satisifing the minimum sample constraint) are {len(self.feasibleClients)}')
        logging.info(f'Online feasible clients at current epoch are {len(clients_online)}')
        ...
        if self.mode == "oort" and self.count > 1:     
            ...
            client_len = min(target_num, len(selection_list))       
