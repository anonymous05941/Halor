...
class Executor(object):
    ...
    def training_handler(self, clientId, conf, feddc_h):
...
#        train_res = client.train(client_data=client_data, model=client_model, conf=conf, round_num=self.epoch) 
        ...
        if self.args.feddc == 1:
            train_res = client.train(client_data=client_data, model=client_model, conf=conf, round_num=self.epoch, feddc_h = feddc_h) 
        else:
            train_res = client.train(client_data=client_data, model=client_model, conf=conf, round_num=self.epoch) 
        ...
        while True:
            if not self.server_event_queue.empty():
                ...
                # initiate each training round
                    ...
                    feddc_h1 = event_dict['feddc_h']
                    train_res = self.training_handler(clientId=clientId, conf=client_conf, feddc_h = feddc_h1) 
                    ...
                    if args.feddc == 1:
                        ...
                        try:
                            ...
                            # 使用 logging.info 输出异常信息
                            logging.info(f"ConnectionResetError: {e}")                     
                    else:
                        self.push_msg_to_server_asyn(event_msg, train_res)


                    # logging.info(f"excutor.py: Executor {self.this_rank}: Client {clientId}, training finish")  
                elif event_msg == 'test':
